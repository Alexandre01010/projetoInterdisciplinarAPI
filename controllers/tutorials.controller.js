...
const getPagination = (page, size) => {
  const limit = size ? Math.abs(size) : 3; // limit = size (default is 3)
  const offset = page ? Math.abs(page) * limit : 0; // offset = page * size (start counting from page 0)
  return { limit, offset };
};
...
exports.findAllComented = (req, res) => {
  //Apenas mostrar informação acerca dos tutoriais
  //Tutorial.findAll({ include: {model: Comment, attributes:{ exclude:['id','author','text','tutorialId']}, required: true }})
  //Apenas mostrar informação dos comentários dos tutoriais
  Tutorial.findAll({attributes: {exclude: ['id','title','description','published']}, include: {model: Comment, required: true }})
    .then((data) => {
      res.status(200).json(data);
    })
    .catch((err) => {
      res.status(500).json({
        message:
          err.message || "Some error occurred while retrieving comented tutorials",
      });
    });
};